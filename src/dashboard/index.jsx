...
export const Dashboard = props => {
  ...
  const [siteData, setSiteData] = useState({
    ...
    domain: 'mywebsite5123468.abcfinancial.net',
  })
  ...
  return (
    <>
      <div className="grid">
        <Card abcId="content1" shadow={false} className="content content1">
          <div>
            ...
            <Text color="secondary">{siteData.description}</Text>
          ...
          <Form.ActionsLayout>
            <Menu>
              {({ isExpanded }) => (
                <>
                  <Menu.Button mode="secondary" rounded>
                    Site Settings
                    {isExpanded ? <IconArrowUp /> : <IconArrowDown />}
                  </Menu.Button>
                  <Menu.List position="matchWidth" className="menuList">
                    <Menu.Item onSelect={() => setSettingsModal(true)}>General</Menu.Item>
                    <Menu.Item onSelect={() => {}}>Fonts and Colors</Menu.Item>
                  </Menu.List>
                </>
              )}
            </Menu>

            <Button mode="primary" rounded onClick={() => setNewPageModal(true)}>
        ...
        <Card abcId="content2" shadow={false} className="content content2">
          ...
          >
            ...
            <Text>
              Start adding pages for your new website. Use a ready-made template to add content
            </Text>
      ...
      <Modal open={settingsModal} title="General Settings" onClose={() => setSettingsModal(false)}>
        <GeneralSettingsForm
      ...
      >
        <Modal.Body abcId="">
          <Text color="secondary">
            Choose a suitable template and customize it to fit your purposes and ideas.
          </Text>
        </Modal.Body>
        ...
        <ElasticGrid
          ...
          renderItem={item => (
            <Card abcId="" className="template">
              <Button abcId="" mode="secondary" size="small" className="templatePreview">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
